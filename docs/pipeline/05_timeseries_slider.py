...
def _write_html(frames, bounds, vmax):
    ...
    conc_path = HERE / "overlay.geojson"  # piletas de evaporación / concesión (opcional)
    conc = json.dumps(json.load(open(conc_path))) if conc_path.exists() else "null"
    html = f"""<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
<title>Deformación acumulada — Salar del Hombre Muerto</title>
...
<div class="panel">
  ...
  <div style="font-size:11px;color:#666;text-align:center">
    Sentinel-1/SBAS 2019–2026 · tropo ERA5 · suavizado · respecto a la 1ª fecha</div>
</div>
<script>
const FR = {data};
const B = [[{s},{w}],[{n},{e}]];
...
L.marker([{aoi.FENIX[0]},{aoi.FENIX[1]}]).addTo(map).bindTooltip('Fénix');
