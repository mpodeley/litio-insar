...
def _add_overlay(folium, m) -> None:
    """Agrega los contornos del overlay (piletas / concesión), sin relleno."""
    import json
    if not OVERLAY.exists():
        return
    folium.GeoJson(
        json.load(open(OVERLAY)), name="Piletas / concesión",
...
def build_html() -> None:
    ...
    _add_overlay(folium, m)
    folium.Marker(
        list(aoi.FENIX), tooltip="Fénix (operación)",
...
def _legend_html(vmax: float) -> str:
    ...
    <div style="position: fixed; bottom: 24px; left: 24px; z-index: 9999;
      ...
      <span style="font-size:11px;color:#555">
        Salar del Hombre Muerto — Sentinel-1/SBAS 2019–2026, tropo ERA5.
