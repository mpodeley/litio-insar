...
OVERLAY = HERE / "overlay.geojson"  # piletas de evaporación / concesión (opcional)


def _plot_overlay(ax) -> None:
    """Dibuja solo los contornos del overlay (piletas / concesión, lon/lat), sin relleno."""
    import json
    if not OVERLAY.exists():
        return
    gj = json.load(open(OVERLAY))
...
def main() -> None:
    ...
    # Contornos del overlay (piletas / concesión, solo polígonos, sin relleno).
    _plot_overlay(ax)

    # Operación Fénix como referencia.
    ax.scatter(*aoi.FENIX[::-1], marker="s", s=70, c="black",
               edgecolor="white", zorder=5, label="Fénix")
    ax.annotate("Fénix", aoi.FENIX[::-1], textcoords="offset points",
                xytext=(6, 4), fontsize=9, color="black", weight="bold")

    ax.set_xlim(w, e); ax.set_ylim(sth, n)
    ax.set_xlabel("Longitud"); ax.set_ylabel("Latitud")
    ax.set_title(
        "Velocidad de deformación del suelo (LOS) — Salar del Hombre Muerto",
        fontsize=13, weight="bold",
    )
    ax.text(
        0.5, 1.012,
        "Sentinel-1 / SBAS 2019–2026 · corrección "
